...
const ConstructionStats = () => { // Renombrado para claridad
  return (
      ...
      <div className="mx-auto max-w-7xl px-4 py-20 md:py-24">
        <div className="grid grid-cols-1 gap-12 md:grid-cols-2">
          ...
          <div className="relative h-full min-h-[400px] overflow-hidden rounded-2xl">
            <img
              src="/construction-stats.jpg" // Aquí deberás colocar la ruta correcta de tu imagen
...
const Stat = ({ num, suffix, decimals = 0, subheading }) => {
  ...
  return (
    ...
    <div className="border-b border-white flex h-full flex-col items-center justify-center p-6 text-center"> 
      {/* Número animado y sufijo */}
      <p className="mb-3 text-5xl font-bold text-yellow-500 md:text-6xl"> 
        {/* El span se llenará con el número animado */}
        <span ref={ref}>{isInView ? num.toFixed(decimals) : '0'}</span> 
        {suffix}
      </p>
      {/* Subtítulo descriptivo */}
      <p className="max-w-xs text-sm text-neutral-300">{subheading}</p> 
    </div>
  );
};
