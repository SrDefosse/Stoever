...
const NavLink = ({ href, children, mobile = false, closeMenu }) => (
  ...
  >
    <Link to={href} onClick={mobile ? closeMenu : undefined}>
      {children}
    </Link>
  </li>
);
          ...
          <Link to="/">
            <div className="flex items-center bg-gray-900/20 backdrop-blur-xl rounded-full py-2 px-6 shadow-lg border border-white/30 transition-transform duration-200 hover:scale-[1.02] cursor-pointer">
              <div className="hidden lg:block">
                <img
                  src="/icons/group_text_icon_white.webp"
                  alt="Stoever Group"
                  width={150}
                  height={40}
                  ...
                  <div key={index}>
                    {link.items ? (
                      <div className="relative">
                        ...
                        {/* Versión móvil del dropdown adaptada */}
                        <div
                ...
                >
                  {link.items ? (
                    <>
                      ...
                      <DropdownMenu items={link.items.map(item => ({...item, to: item.href}))} isOpen={activeDropdown === index} />
                    </>
                  ) : (
